...
public class FirmwareSession
{
    ...
    public async Task<string?> SendCommandAsync(IFirmwareRequest request, TimeSpan timeout)
    {
        return await Task.Run(() =>
            SendCommand(request, timeout)
        );
    }

    public async Task<T?> SendCommandAsync<T>(IFirmwareRequest<T> request, TimeSpan timeout)
    {
        return await Task.Run(() =>
            SendCommand(request, timeout)
        );
